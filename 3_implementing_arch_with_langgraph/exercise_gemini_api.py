...
# Función helper para extraer JSON de markdown
...
llm= ChatGoogleGenerativeAI(model= "gemini-2.5-flash-lite", temperature= 0.0, google_api_key= os.getenv("AI_AGENTIC_API_KEY") )
...
def intake_node(state: BlogState)-> BlogState:
    ...
    json_content = extract_json(last_ai_message.content)
    parser = PydanticOutputParser(pydantic_object=Blogplan)
    structured_output: Blogplan = parser.parse(json_content)

    print(f"message: {last_ai_message.content[:50]}...")
    print("Done!\n")
...
def research_node(state: BlogState)-> BlogState:
    ...
    structured_output: ResearchData = parser.parse(json_content)
...
def writer_node(state: BlogState):
    ...
    json_content = extract_json(last_ai_message.content)
    parser = PydanticOutputParser(pydantic_object=ContentDraft)
    structured_output: ContentDraft = parser.parse(json_content)
...
def reviewer_node(state: BlogState) -> BlogState:
    ...
    json_content = extract_json(last_ai_message.content)
    parser = PydanticOutputParser(pydantic_object=ReviewedContent)
    structured_output: ReviewedContent = parser.parse(json_content)
...
def publisher_node(state: BlogState) -> BlogState:
    ...
    last_ai_message:AIMessage = messages[-1]
    json_content = extract_json(last_ai_message.content)    
    parser = PydanticOutputParser(pydantic_object=PublicationStatus)
    structured_output: PublicationStatus = parser.parse(json_content)
